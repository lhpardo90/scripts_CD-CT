...
if [[ $MODERUN == "R" ]]; then
   ...
   for ((hour=0; hour<=FCST; hour+=dt)); do
      ...
      cp -f "${BNDDIR}/era5.pl.${valid_date}.grib" "${DATAIN}/${RUN_ID}/"
      cp -f "${BNDDIR}/era5.sl.${valid_date}.grib" "${DATAIN}/${RUN_ID}/"
   done
...
./link_grib.csh ${DATAIN}/${RUN_ID}/era5.*.grib
...
EOF0
   ...
   cp -f ${BNDDIR}/era5.pl.${YYYYMMDDHHi}.grib ${DATAIN}/${RUN_ID}
   cp -f ${BNDDIR}/era5.sl.${YYYYMMDDHHi}.grib ${DATAIN}/${RUN_ID}
...
./link_grib.csh ${DATAIN}/${RUN_ID}/era5.*.${YYYYMMDDHHi}.grib
...
#
   mv ungrib.log ${DATAOUT}/${RUN_ID}/Pre/logs/ungrib.${start_date}.log
   mv namelist.wps ${DATAOUT}/${RUN_ID}/Pre/logs/namelist.${start_date}.wps
   mv ${EXP}\:${start_date:0:13} ${DATAOUT}/${RUN_ID}/Pre
   rm -fr ${DATAIN}/${RUN_ID}
